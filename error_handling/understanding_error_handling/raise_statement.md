...
You can see here that `raise` is the keyword that triggers an exception. In this example, we're raising a `ValueError` with a custom message when an invalid age is provided.
...
Here the keyword `raise` (without arguments) re-raises the current exception that's being handled. This allows you to log or perform cleanup while still propagating the error up the call stack.
...
Here you can see custom exception classes inherit from `Exception` or its subclasses. You'll learn more about classes and inheritance in future lessons. For now, know that this is a way to create your own exceptions with custom logic.
